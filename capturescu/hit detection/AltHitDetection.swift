...
func isPointNearPathAlt(testPoint: CGPoint, path: Path, threshold: CGFloat = 20) -> BoundingBox? {
    // Step 1: Extract points from the path
    ...
    // Step 3: Quick check to see if the point is within the bounding box
    if !isPointInBoundingBox(point: testPoint, boundingBox: boundingBox) {
        return nil // The point is outside the bounding box
    }

    // Step 4: If within bounding box, check distance to each line segment
